...
import { LONGS_SCENE_ID } from '../../app.constants';
...
@Scene(LONGS_SCENE_ID)
export class LongScene {
	...
	async onSceneEnter(@Ctx() ctx: IContext): Promise<void> {
		await ctx.reply(ctx.i18.t('Long.restrictions'), Back(ctx));
	}
	...
	async onDownloadShorts(
		...
		try {
			...
			if (!isDuration) {
				await ctx.reply(ctx2.i18.t('Long.duration'), Back(ctx2));
				return;
			}
			...
			await this.analyticsService.createError('long');
